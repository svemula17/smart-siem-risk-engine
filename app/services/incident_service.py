...
def update_incident_status(db: Session, incident_id: str, new_status: str, user_name: str = "System") -> IncidentDB | None:
...
def assign_incident(db: Session, incident_id: str, user_id: int, user_name: str) -> IncidentDB | None:
    incident = db.query(IncidentDB).filter(IncidentDB.id == incident_id).first()
    if not incident:
        return None
