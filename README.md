...
* **📚 Interactive Developer API Portal:** A dedicated, fully-featured API documentation page (`/api-docs`) built with a premium glassmorphic UI. It features real-time client-side search, one-click endpoint path copying, responsive mobile design, and a dark/light theme toggle. The dashboard top navigation integrates a convenient dropdown to directly access API categories (Alert Endpoints, Actions, Metrics), empowering developers to seamlessly connect the Risk Engine with external SOC systems.
